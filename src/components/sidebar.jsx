...
const Sidebar = () => {
  ...
  return (
    <aside className="w-full h-fit md:w-[274px] md:h-[568px] relative text-white">
        ...
        {steps.map((step, i) => {
          return (
            <div key={i} className="flex items-center md:gap-4">
              <div className={`w-[33px] h-[33px] rounded-full border-2 flex items-center justify-center text-sm font-semibold ${step.number === currentStep && 'bg-blue-100 text-blue-marine'}`}>{step.number}</div>
